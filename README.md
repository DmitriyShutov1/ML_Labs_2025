...
На основе проведенных исследований можно сформулировать следующие практические рекомендации. Для достижения максимального качества в задачах классификации следует использовать Gradient Boosting с тщательным подбором гиперпараметров. Для регрессионных задач с ярко выраженными локальными зависимостями эффективным выбором может быть KNN с правильной нормализацией признаков.

Random Forest представляет собой хороший компромисс между качеством, скоростью обучения и устойчивостью к переобучению, что делает его подходящим выбором для большинства практических задач. Decision Tree после соответствующей регуляризации может быть полезен в случаях, когда важна интерпретируемость модели.

Важным выводом является необходимость адаптации методов улучшения к специфике алгоритма: для деревьев критически важна регуляризация, для бустинга - тонкая настройка гиперпараметров, для линейных моделей - правильный препроцессинг данных.
...
Наиболее ценным результатом работы является не столько конкретные численные показатели моделей, сколько выработанный системный подход к исследованию алгоритмов, умение анализировать результаты и делать обоснованные выводы о применимости тех или иных методов в различных практических ситуациях.
---
